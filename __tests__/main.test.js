...
/** @type {ReadonlyArray<[columnName: string, keyword?: string, options?: import("postgraphile/presets/v4").V4Options & GraphileBuild.SchemaOptions, testOptions?: {fk_children_parents?: boolean}]>} */
const tests = [
  ["default"],
  [
    "default_w_comment",
...
describe.each(tests)(
  "%s",
  (_columnName, keyword, graphileBuildOptions, config = {}) => {
    const { fk_children_parents = null } = config;
    /** @type {'Archived'} */
    const Keyword = keyword
      ? /** @type {'Archived'} */ (keyword[0].toUpperCase() + keyword.slice(1))
      : `Archived`;
    /** @type {import('graphql').GraphQLSchema} */
    let schema;
    /** @type {GraphileConfig.ResolvedPreset} */
    let resolvedPreset;
    /** @type {'pgArchivedRelations'} */
...
comment on constraint fk_children_parents on omit_archived.children is E'@archivedRelation';`);
      ...
      const preset = {
        ...
        pgServices: [
          makePgService({
            pool: pgPool,
            schemas: ["omit_archived"],
          }),
        ],
      };

      // Build schema
      ({ schema, resolvedPreset } = await makeSchema(preset));
    });
